  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: '0.75rem' }}>
        {[
          ...
          <div key={s.label} className="stat-card" style={{ padding: '0.75rem' }}>
            <span style={{ fontSize: '0.63rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--text-muted)', marginBottom: '0.25rem', display: 'block' }}>
              {s.label}
            </span>
            <span style={{ fontSize: '1.4rem', fontWeight: 800, color: s.color, lineHeight: 1, display: 'block' }}>
              {s.value}
            </span>
          </div>
        ))}
      ...
      <div className="dashboard-grid-2">
        ...
        {/* LEFT: Team Workload Table (main section) */}
                    ...
                    <tr key={m.id}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <div className="avatar" style={{ backgroundColor: m.avatarColor || '#3b82f6' }}>
                      ...
                      <td style={{ textAlign: 'center', fontWeight: 700, color: 'var(--text-primary)' }}>{m.assigned}</td>
                      <td style={{ textAlign: 'center', color: 'var(--status-todo-text)' }}>{m.todo}</td>
                      <td style={{ textAlign: 'center', color: 'var(--status-progress-text)' }}>{m.inProgress}</td>
                      <td style={{ textAlign: 'center', color: 'var(--status-review-text)' }}>{m.inReview}</td>
                      <td style={{ textAlign: 'center', color: 'var(--status-done-text)' }}>{m.done}</td>
                      <td style={{ textAlign: 'center', color: m.overdue > 0 ? 'var(--priority-critical-text)' : 'var(--text-muted)', fontWeight: m.overdue > 0 ? 700 : 400 }}>
                        {m.overdue}
                      </td>
                      <td style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.78rem' }}>{m.estimatedHours}h</td>
                      <td style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.78rem' }}>{m.loggedHours}h</td>
                      <td>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.5rem', marginBottom: '0.875rem' }}>
                {[
                  ...
                  <div key={s.label} style={{
                    ...
                    <div style={{ fontSize: '0.6rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.15rem' }}>
                      {s.label}
                    </div>
                  ...
                  {myTasks.slice(0, 10).map(t => (
                    <div key={t.id} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', minWidth: 0, flex: 1 }}>
                        <span style={{ fontFamily: 'monospace', fontSize: '0.68rem', color: 'var(--text-muted)', flexShrink: 0 }}>
                          {t.id}
                        </span>
